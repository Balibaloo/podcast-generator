...
module.exports.generate = (directoryPath, img_path, mainCallback) => {
    ...
    let writeToTxt = (fileList,callback) => {
        ...
        fileList = fileList.map((line) => {
            let pat = (directoryPath+"\\"+line).replace(" ","\ ") //formatPath() 
            console.log(pat)
            return `file ${pat}`})
    ...
    fileList = fileList.map((line) => {
        let pat =  directoryPath+"\\"+line //formatPath()
        console.log(pat)
        return `"${pat}"`})
    ...
    -i MA.jpg \
    -i ${fileList.join(" -i ")} \
    -filter_complex "[0]scale='iw-mod(iw,2)':'ih-mod(ih,2)',format=yuv420p[v];${randomBullshitArguments}concat=n=${fileList.length}:v=0:a=1[a]" \
    -map "[v]" -r 15 -map "[a]" \
