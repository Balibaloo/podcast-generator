...
module.exports.generate = (directoryPath, img_path, mainCallback) => {
    ...
    exec(`ffmpeg -loop 1 -framerate 1 -i MA.jpg -i ${fileList.join(" -i ")} -filter_complex "[0]scale='iw-mod(iw,2)':'ih-mod(ih,2)',format=yuv420p[v];${randomBullshitArguments}concat=n=${fileList.length}:v=0:a=1[a]" -map "[v]" -r 15 -map "[a]" -tune stillimage -shortest ${outputFilename}`,(error,stdout,stderr) => {
        if (error){
            console.log(error)
