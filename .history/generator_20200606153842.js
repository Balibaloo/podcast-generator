...
let concatAudioFiles = (fileList, callback) => {
    let outputFilename = fileList[0].split("_").slice(0,-1).join("_")

    console.log(`ffmpeg -i 'concat:${fileList.join("|")}' -c copy ${outputFilename}.mp3`)

    //exec(`ffmpeg -i 'concat:${fileList.join("|")}' -c copy ${outputFilename}.mp3`)
