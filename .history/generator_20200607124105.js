...
module.exports.generate = (directoryPath, mainCallback) => {
    ...
    exec(`ffmpeg -loop 1 -framerate 1 -i MA.jpg -i ${fileList.join(" -i ")} -filter_complex 'concat=n=${fileList.length}:v=0:a=${fileList.length}' -tune stillimage -shortest ${outputFilename}`,(error,stdout,stderr) => {
        if (error){
            console.log(error)
