...
let getNameRoot = (directoryPath) => {
    console.log(directoryPath.split("\\").slice(-1)[0].slice(0,-4))
    return directoryPath + directoryPath.split("\\").slice(-1).slice(0,-4)
}
...
module.exports.generate = (directoryPath, mainCallback) => {
    ...
    console.log(fileNameRoot)

    let writeToTxt = (fileList,callback) => {

        fileNameRoot += "\\" + fileList[0].slice(0,-7)
        let fileName = fileNameRoot + "_playlist.txt"

        console.log("txt filename", fileName)

        fileList = fileList.map((line) => {return `file ${formatPath(directoryPath+"\\"+line)}`})
