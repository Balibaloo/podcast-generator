...
module.exports.generate = (directoryPath, img_path, mainCallback) => {
    ...
    //`ffmpeg -loop 1 -i MA.jpg -i "concat:${fileList.join("|")}" -c:a copy -c:v libx264 -shortest ${outputFilename}`
        `ffmpeg -loop 1 -framerate 1 -i MA.jpg -i file1.mp3 -i file2.mp3 -i  -filter_complex"[0]scale='iw-mod(iw,2)':'ih-mod(ih,2)',format=yuv420p[v];[1:a][2:a][3:a]           concat=n=                 3:v=0:a=1[a]" -map "[v]" -r 15 -map "[a]" -tune stillimage -shortest -movflags +faststart output.mp4`
    exec(`ffmpeg -loop 1 -framerate 1 -i MA.jpg -i ${fileList.join(" -i ")} -filter_complex "[0]scale='iw-mod(iw,2)':'ih-mod(ih,2)',format=yuv420p[v];${randomBullshitArguments}concat=n=${fileList.length}:v=0:a=1[a]" -map "[v]" -r 15 -map "[a]" -tune stillimage -shortest -movflags +faststart ${outputFilename}`,(error,stdout,stderr) => {
        if (error){
            console.log(error)
